...
<body>
	...
	<div class="container">
		<main class="main">
			...
			$select = "SELECT * FROM payments where idAuthor = :iduser";
			$payments = $connection->prepare("$select");
			$payments->bindValue(":iduser", $_SESSION['login']);
			$payments->execute();
			echo '<div class="payments">
			<h2 class="payments_title">История платежей за все время</h2>';
			if ($payments->rowCount() > 0) {
				echo '<div class="payments__body">';
				foreach ($payments as $payment) {
					echo '
					<p class="payments__date">Дата платежа: '. $payment["date"] .'</p>
					<p class="payments__cost">Сумма: '. $payment["cost"] .'₽</p>';
				}
				echo '</div>';
			}
			echo '</div>';
